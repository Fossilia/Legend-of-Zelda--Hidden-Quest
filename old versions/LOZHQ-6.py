...
def addEnemy(enemy, num, x, y, stop, speed, length, health):
    ...
    global epics, eDelay, eFrame, guy, eX, eY, eTime, eTurn, randX, randY, enemyXY,eRects, playerRect, dire, push, eHealth, eDmg
...
def collide(rect, dmg, num):
    global health, attack, push, eDmg, eHealth, frame2
    ...
    if eHealth[num]>0:
        ...
        if attack == True:
            sound = mixer.Sound("sounds/Hit.wav")
            sound.play()
            if dire == 14:
                push[num][X] += 30
                eDmg[num]+=1
...
#attack------------------------------------------------------
    ...
    if keys[K_SPACE]:
        ...
        sound = mixer.Sound("sounds/Swoosh.wav")
        sound.play()
